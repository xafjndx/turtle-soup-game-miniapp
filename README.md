...
| 改进项 | 状态 | 说明 |
|--------|------|------|
| 首次启动隐私政策 | ✅ 已实现 | 首次启动强制展示，必须同意才能使用 |
...
| 数据管理功能 | ✅ 已实现 | 支持查看、导出、删除个人信息，注销账号 |

### 题目来源（合规方式）

1. **管理员添加** - 通过管理后台手动添加
2. **AI辅助生成** - 生成后需人工审核才能上架
3. **用户投稿** - 用户可在小程序内投稿，需人工审核
...
| 题库管理 | 分类抽取、AI生成、用户投稿（均需人工审核） |
| 排行榜 | 命中率排名、实时更新 |
| 用户投稿 | 在线投稿、审核流程 |
| 后台管理 | 数据统计、题目审核、操作日志 |
| 隐私保护 | 隐私政策同意、数据管理、注销账号 |
...
| /api/user/isAdmin | GET | 检查是否管理员 |
| /api/user/delete | DELETE | 注销账号 |
| /api/user/export | GET | 导出个人数据 |
...
| /api/question/submit | POST | 用户投稿题目 |
...
- 位置：个人中心页面
- 仅管理员用户可见
- 功能：统计概览、待审核题目、审核通过/拒绝

### Web 管理后台
访问 `admin/index.html` 打开完整管理后台：
...
投稿的题目默认状态为 **PENDING**，需要管理员审核通过后才能加入题库。
...
-- 设置管理员
INSERT INTO admins (id, userId, role, permissions, createdAt)
VALUES ('admin-001', '用户ID', 'SUPER', '["ALL"]', datetime('now'));
```
